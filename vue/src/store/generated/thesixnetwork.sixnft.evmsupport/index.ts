...
import { AddressBinding } from "thesixnetwork-sixnft-client-ts/thesixnetwork.sixnft.evmsupport/types"
import { Params } from "thesixnetwork-sixnft-client-ts/thesixnetwork.sixnft.evmsupport/types"
	...
	actions: {
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.ThesixnetworkSixnftEvmsupport.query.queryParams()).data
		...
		async QueryAddressBinding({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ThesixnetworkSixnftEvmsupport.query.queryAddressBinding( key.ethAddress,  key.nativeAddress)).data
		...
		async QueryAddressBindingAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ThesixnetworkSixnftEvmsupport.query.queryAddressBindingAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.ThesixnetworkSixnftEvmsupport.query.queryAddressBindingAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgRemoveBinding({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.ThesixnetworkSixnftEvmsupport.tx.sendMsgRemoveBinding({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgBindAddress({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.ThesixnetworkSixnftEvmsupport.tx.sendMsgBindAddress({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgRemoveBinding({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ThesixnetworkSixnftEvmsupport.tx.msgRemoveBinding({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgRemoveBinding:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgBindAddress({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ThesixnetworkSixnftEvmsupport.tx.msgBindAddress({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgBindAddress:Init Could not initialize signing client. Wallet is required.')
