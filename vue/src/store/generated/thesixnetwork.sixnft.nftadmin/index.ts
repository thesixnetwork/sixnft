...
import { Authorization } from "thesixnetwork-sixnft-client-ts/thesixnetwork.sixnft.nftadmin/types"
import { Params } from "thesixnetwork-sixnft-client-ts/thesixnetwork.sixnft.nftadmin/types"
import { AddressList } from "thesixnetwork-sixnft-client-ts/thesixnetwork.sixnft.nftadmin/types"
import { Permissions } from "thesixnetwork-sixnft-client-ts/thesixnetwork.sixnft.nftadmin/types"
	...
	actions: {
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.ThesixnetworkSixnftNftadmin.query.queryParams()).data
		...
		async QueryAuthorization({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ThesixnetworkSixnftNftadmin.query.queryAuthorization()).data
		...
		async sendMsgBurn({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.ThesixnetworkSixnftNftadmin.tx.sendMsgBurn({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgGrantPermission({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.ThesixnetworkSixnftNftadmin.tx.sendMsgGrantPermission({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgRevokePermission({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.ThesixnetworkSixnftNftadmin.tx.sendMsgRevokePermission({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgMint({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.ThesixnetworkSixnftNftadmin.tx.sendMsgMint({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgBurn({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ThesixnetworkSixnftNftadmin.tx.msgBurn({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgBurn:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgGrantPermission({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ThesixnetworkSixnftNftadmin.tx.msgGrantPermission({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgGrantPermission:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgRevokePermission({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ThesixnetworkSixnftNftadmin.tx.msgRevokePermission({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgRevokePermission:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgMint({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ThesixnetworkSixnftNftadmin.tx.msgMint({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgMint:Init Could not initialize signing client. Wallet is required.')
