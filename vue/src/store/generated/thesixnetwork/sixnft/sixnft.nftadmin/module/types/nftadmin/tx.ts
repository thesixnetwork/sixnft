...
export const protobufPackage = "sixnft.nftadmin";
...
export class MsgClientImpl implements Msg {
  ...
  Mint(request: MsgMint): Promise<MsgMintResponse> {
    ...
    const promise = this.rpc.request("sixnft.nftadmin.Msg", "Mint", data);
    return promise.then((data) => MsgMintResponse.decode(new Reader(data)));
  }

  Burn(request: MsgBurn): Promise<MsgBurnResponse> {
    const data = MsgBurn.encode(request).finish();
    const promise = this.rpc.request("sixnft.nftadmin.Msg", "Burn", data);
