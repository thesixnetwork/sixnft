...
case $choice in
    ...
    3) echo "Mockup Token"
        ...
        sixnftd tx nftmngr create-metadata "${schema_code}" ${token_id} --from alice --gas auto --gas-adjustment 1.5 --gas-prices 0.1stake -y --node http://sixnft.fivenet.sixprotocol.net:26657 \
            ${BASE64_META}
        ;;
    4) echo "Do Action"
        read -p "Enter Schema Code: " schema_code 
